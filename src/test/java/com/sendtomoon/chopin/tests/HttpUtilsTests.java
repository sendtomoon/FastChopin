...
public class HttpUtilsTests {
	...
	public void postTest() {
		...
		try {
			...
			map = HttpUtils.post("http://192.168.0.1/ajax_status.xml?hash=0.24395379430691233", "", null, getH2(),
					cookie);
			System.err.println(map.get("response"));
		} catch (Exception e) {
			e.printStackTrace();
	...
	private Map<String, String> getH2() {
		Map<String, String> header = new HashMap<String, String>();
		header.put("Accept",
				"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3");
		header.put("Accept-Encoding", "gzip, deflate");
		...
		header.put("Origin", "http://192.168.0.1");
		header.put("Proxy-Connection", "keep-alive");
		header.put("Referer", "http://192.168.0.1/login.cgi");
		header.put("Upgrade-Insecure-Requests", "1");
		header.put("User-Agent",
				"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.86 Safari/537.36");
		return header;
